...
.PHONY: help
help:
	@awk -F':' '/^[a-zA-Z0-9_-]+:/ {printf "\033[36m%-20s\n",$$1}' $(MAKEFILE_LIST)
...
deploy:
	...
	gcloud beta run deploy ednrag \
		...
		--update-env-vars \
		"DATA_STORE_ID=ednrag-collection_documents,DATA_STORE_REGION=global" \
		$(if $(IAP),--iap) \
		$(if $(PORT),--port=$(PORT))
...
data-ingestion:
	...
	gcloud storage cp sample_data/* gs://$$PROJECT_ID-ednrag-docs/ && \
	uv run deployment/terraform/scripts/start_connector_run.py $$PROJECT_ID $$DATA_STORE_REGION ednrag-collection --wait
